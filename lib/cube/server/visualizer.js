...
exports.register = function(db, endpoints) {
  ...
  endpoints.http.push(
    ...
    endpoint.re(/^\/themes\/.*/, endpoint.dir("../../../themes", /^\/themes\/(.*)/)),
...
function directoryForTheme(theme) {
  var themeBase = path.normalize(path.join(__dirname, "..", "..", "..", "themes"));
  var themeDir = path.normalize(path.join(themeBase, theme));
  if(themeDir.indexOf(themeBase) == 0) {
    return themeDir;
  } else {
    return path.join(themeBase, "default");
  }
}
  ...
  return function random(request, response) {
    ...
    boards.findOne({_id: id}, function(error, object) {
      if (object == null) {
        ...
        theme = request.url.indexOf("/edit") < 0 && object.theme || "default";
        var themeDir = directoryForTheme(theme);
        file = endpoint.file(path.join(themeDir, "body.html"));
