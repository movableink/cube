...
exports.register = function(db, endpoints) {
  endpoints.ws.push(
    endpoint.re(/^\/board\/[0-9][0-9a-z]{5}/, viewBoard(db)),
  ...
  endpoints.http.push(
    ...
    endpoint.re(/^\/[0-9][0-9a-z]{5}(\/export)?$/,  loadBoardData(db)),
    endpoint.re(/^\/[0-9][0-9a-z]{5}(\/import)?$/,  importBoardData(db)),
    endpoint.exact("/cube.js", endpoint.file(
      ...
      resolve("boards.js"),
      resolve("header.js"),
      resolve("../../../node_modules/socket.io/node_modules/socket.io-client/dist/socket.io.js"),
      resolve("end.js")
    )),
...
function importBoardData(db) {
  var boards,
      file = endpoint.file(resolve("import.html"));

  db.collection("boards", function(error, collection) {
    boards = collection;
  });

  return function random(request, response, data) {

    var slug  = url.parse(request.url).pathname.substring(1).split('/')[0],
        id    = parseInt(slug, "36");
    ...
    boards.findOne({_id: id}, function(error, object) {
      ...
      if (object === null) {
        ...
        var newObject = JSON.parse(data.import_data);
        delete newObject._id;

        // update the dashboard
        // TODO add error checking and versioning
        boards.update({_id: id}, newObject);

        // Redirect to view dashboard
        response.statusCode = 302;
        response.setHeader("Location", "/" + slug);
        return response.end();
