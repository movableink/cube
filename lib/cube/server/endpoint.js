...
function dir(directory, re) {
  var base = path.normalize(path.join(__dirname, directory));

  return function(request, response) {
    var match = request.url.match(re)[1];
    if(!match) {
      return fiveohoh(request, response);
    }

    var asset = path.normalize(path.join(base, match));
    if(asset.indexOf(base) == 0) {
      file(asset)(request, response);
    } else {
      fiveohoh(request, response);
    }
  }
}
