cube.board = function(host, path, id) {
  var board = {id: cube_board_formatId(id)},
      ...
      squares,
      pieces = [],
      size = [32, 68], // in number of squares
  ...
  function open() {
    ...
    socket.on('connect', function() {
      ...
      socket.on([path, socketId].join('::'), message);
      load();
    });
  }

  function load() {
    if (id && socket && socket.socket.connected) {
      socket.send(JSON.stringify({type: "load", id: id, endpoint: path, socket_id: socketId}));
    }
  }

  // A one-time listener to send an add event on mouseup.
  function add() {
    socket.send(JSON.stringify({type: "add", id: id, piece: this, endpoint: path, socket_id: socketId}));
    this.on("move.board", move);
  }

  function move() {
    socket.send(JSON.stringify({type: "move", id: id, piece: this, endpoint: path, socket_id: socketId}));
  }

  function edit() {
    socket.send(JSON.stringify({type: "edit", id: id, piece: this, endpoint: path, socket_id: socketId}));
  }
  ...
  board.remove = function(piece, silent) {
    ...
    if (silent) {
      ...
      socket.send(JSON.stringify({type: "remove", id: id, piece: {id: piece.id}, endpoint: path, socket_id: socketId}));
