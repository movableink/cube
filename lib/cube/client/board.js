cube.board = function(host, path, id) {
  ...
  var endpoint = [path, board.id].join('/');
    ...
    switch (e.type) {
      ...
      case "add": {
        for(var i = 0; i < pieces.length; i++) {
          if(pieces[i].id == e.piece.id) { return; }
        }
  ...
  function load() {
    if (id && socket && socket.socket.connected) {
      socket.send(JSON.stringify({type: "load", id: id, endpoint: endpoint}));
    }
  }

  // A one-time listener to send an add event on mouseup.
  function add() {
    socket.send(JSON.stringify({type: "add", id: id, piece: this, endpoint: endpoint}));
    this.on("move.board", move);
  }

  function move() {
    socket.send(JSON.stringify({type: "move", id: id, piece: this, endpoint: endpoint}));
  }

  function edit() {
    socket.send(JSON.stringify({type: "edit", id: id, piece: this, endpoint: endpoint}));
  }
  ...
  board.remove = function(piece, silent) {
    ...
    if (silent) {
      ...
      socket.send(JSON.stringify({type: "remove", id: id, piece: {id: piece.id}, endpoint: endpoint}));
